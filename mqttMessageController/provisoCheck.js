    ...
    if(conditions && (timeNow || anyTime) ){
        for (let i = 0; i < conditions.length; i++) {
                ...
                if (topic.elementId == conditions[i].sensor){
                    console.log(await compareByProviso(conditions[i][checkedValueKey], conditions[i].proviso, getSend[checkedValueKey]))
...
async function compareByProviso(ShellduesValue, proviso, getSendValue){
    ...
    console.log(ShellduesValue)
    console.log(getSendValue),
    console.log(proviso)
    switch (proviso) {
        case "=":
